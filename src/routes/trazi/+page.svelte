<script>
  ...
  $: idx = searchIndexes
    ? lunr.Index.load(typeof searchIndexes === 'string' ? JSON.parse(searchIndexes) : searchIndexes)
    : null;
  ...
  function handleSubmit() {
    ...
    let toSearch = searchField.trim().toLowerCase();

    replaces.forEach((it) => (toSearch = toSearch.replace(new RegExp(it.value, 'g'), it.replace)));

    results = idx
      .search(toSearch)
      .map((it) => search.find((sr) => sr.id === it.ref))
      .filter(Boolean);
  }
</script>
